...
export PATH=/home/scriptdevil/FakeRoot/bin/:$PATH
...
herbstclient pad 0 0 0 $panel_height
{
    # events:
    #mpc idleloop player &
    while true ; do
    ...
    while true; do
        echo "battery $(acpi -bi | head -n1 | awk '{print $3 $4 $5}' | tr -d '\n')"
        sleep 1 || break
    done > >(uniq_linebuffered) &
    childpid=$!
    herbstclient --idle
    ...
    while true ; do
        ...
        # small adjustments
        right="$separator^bg() $date $separator $battery"
...
} 2> /dev/null | dzen2 -w $panel_width -x $x -y 752  -fn "$font" -h $panel_height \
